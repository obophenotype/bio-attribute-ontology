...
#####################################################
### DOSDP definitions post-processing ###############
#####################################################

# Overrides the autogenerated definitions.owl rule to run a SPARQL update
# (../sparql/postprocess-definitions.ru) after merging the pattern modules.
# This strips the placeholder "anatomical entity" location artefacts from the
# generic amount/level terms generated by entity_attribute_location.
# See https://github.com/obophenotype/bio-attribute-ontology/issues/439
# It also settles the generated entity-first and "level" synonyms of amount
# traits (see the header of postprocess-definitions.ru).
$(PATTERNDIR)/definitions.owl: $(DOSDP_OWL_FILES_DEFAULT)
	if [ "${DOSDP_PATTERN_NAMES_DEFAULT}" ] && [ $(PAT) = true ]; then $(ROBOT) merge $(addprefix -i , $^) \
		query --update ../sparql/postprocess-definitions.ru \
